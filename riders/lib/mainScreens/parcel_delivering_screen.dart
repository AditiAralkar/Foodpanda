...
class _ParcelDeliveringScreenState extends State<ParcelDeliveringScreen> {
  ...
  confirmParcelHasBeenDelivered(getOrderId, sellerId, purchaserId, purchaserAddress, purchaserLat, purchaserLng)
  {
    ...
    }).then((value)
    {
      FirebaseFirestore.instance
          ...
          .update({
        "earnings": riderNewTotalEarningAmount, //total earning amount of rider
      });
    }).then((value)
    {
      FirebaseFirestore.instance
        ...
        ({
        "earnings": (double.parse(orderTotalAmount)  + (double.parse(previousEarnings))).toString(), //total earning amount of seller
      });
      }).then((value)
    {
      FirebaseFirestore.instance
  ...
  {
    FirebaseFirestore.instance
        .collection("orders").doc(widget.getOrderId)
        .get()
      .then((snap)
        {
